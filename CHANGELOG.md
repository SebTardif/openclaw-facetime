...
- Reject non-string `callUUID` values on authenticated call-control actions before asking TelephonyUtilities for a call, so JSON null cannot crash FaceTime. Handshake and envelope string fields are class-checked the same way before `isEqualToString:`.
